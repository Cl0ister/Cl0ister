...
import xbmcvfs
import traceback
import cookielib,xbmcplugin,xbmcgui,xbmc,xbmcaddon,datetime,xbmcgui
...
addon = xbmcaddon.Addon('plugin.video.cloister')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.cloister-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Cloister,failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Cloister,failed to reach the server. - "+str(e.reason)+",10000,"+icon+")")
...
def getItems(items,fanart):
        ...
        for item in items:
            ...
            try:
                if item('epg-pt'):
                    httpr = str(makeRequest(getepgpturl(item('epg-pt')[0].string)))
                    BreakPoint0 = httpr.split(str("<Title>"))[1]
                    EPG = str(BreakPoint0.split(str("</Title>"))[0])
                    name += "[COLOR=white][B][I] ["+str(EPG).decode('utf-8')+"] [/I][/B][/COLOR]"
                else:
                    pass
            except:
                addon_log('EPG-pt Error')


            try:
                if item('subs'):
                    if len(item('subs')[0]) > 0:
                        nice = str(item('subs')[0]).replace('<subs>','')
                        nice = nice.replace('</subs>','')
                        legendas+=nice
                else:
                    pass
            except:
            ...
            try:
                if len(url) > 1:
                    ...
                    if isXMLSource:
                        ...
                        addLink(url[0],name.encode('utf-8', 'ignore'),thumbnail,fanArt,desc,genre,date,True,None,regexs,total,legendas)
                        legendas=''
                    #print 'success'
            except:
...
def playmedia(media_url):
    try:
        ...
        print "Im Here and subs showd be : "+SUBS
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,legendas,setCookie="",allinfo={}):
        #print 'url,name',url,name
        #print "My name is : "+name+" my subs are : "+legendas
        contextMenu =[]
        try:
        ...
        if playlist:
            ...
            if len(legendas)>0:
                u += "url="+urllib.quote_plus(url)+"&subs="+legendas+"&mode="+mode
            else:
                u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
            u += "&regexs="+regexs
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Cloisters Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Cloister Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
def playsetresolved(url,name,iconimage,legenda,setresolved=True):
    if setresolved:
		...
		#xbmcPlayer.play(int(sys.argv[1]),liz)
        xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, liz)
        try:
            if len(legenda)>0:
                xbmc.executebuiltin("XBMC.Notification(Cloister,A Trabalhar as Legendas,1000,"+icon+")")
                xbmc.sleep(5000)
...
def msgbox(texto,tempo):
    xbmc.executebuiltin("XBMC.Notification(Cloister,"+str(texto)+","+str(tempo)+","+icon+")")

def getepgpturl(canal):
    epgurl="http://services.sapo.pt/EPG/GetChannelListByDateInterval?channelSiglas="
    dat = datetime.datetime.now()
    data = str(dat).split(' ')[0]
    datay = data.split('-')[0]
    datam = data.split('-')[1]
    datad = data.split('-')[2]
    databoa = datad+"-"+datam+"-"+datay
    hora = str(dat).split(' ')[1]
    hora = hora.split('.')[0]
    hora = hora.split(':')[0]+":"+hora.split(':')[1]
    replacers=str(canal)+"&startDate="+databoa+"%20"+hora+":01&endDate="+databoa+"%20"+hora+":02"
    epgurl+=replacers
    return epgurl
...
if mode==None:
    ...
    playsetresolved (urlsolver(url),name,iconimage,legenda,True)
